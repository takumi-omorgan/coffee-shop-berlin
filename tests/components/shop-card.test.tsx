...
vi.mock("@/lib/data", () => ({
  getShops: vi.fn(async () => [noteShop]),
  getShopBySlug: vi.fn(async (slug: string) => (slug === noteShop.slug ? noteShop : null)),
  getNeighborhoods: vi.fn(async () => []),
  getNeighborhood: vi.fn(async () => null),
  getShopsByNeighborhood: vi.fn(async () => []),
}));
...
import CafePage from "@/app/cafes/[slug]/page";
...
describe("ShopCard", () => {
  ...
  it("cafe page renders the editor note when present", async () => {
    const jsx = await CafePage({ params: Promise.resolve({ slug: "note-shop" }) });
    render(jsx);
    expect(screen.getByText("Pioneer of the Berlin third-wave scene.")).toBeDefined();
  });
